...
extension AndroidOperationCommand {
    ...
    func runToolchainCommand(_ tc: ToolchainPaths, executable: String?, testMode: TestingMode?, separateScratchPerArch: Bool = false, with out: MessageQueue) async throws -> (cmd: [String], env: [String: String], binPath: String?, buildOutputFallback: String) {
        ...
        // Pin the SwiftPM build engine instead of inheriting the toolchain default, which
        // changed from `native` to `swiftbuild` in Swift 6.4. An explicit `--build-system`
        // in the trailing arguments (e.g. from a gradle file generated by an older Skip)
        // still wins, and is passed through with the rest of `args` below.
        let buildSystem = await resolvedBuildSystem(swift: swiftCmd)
        cmd += ["--build-system", buildSystem.rawValue]

        // The swiftbuild engine writes every architecture into the same product directory
        // (<scratch>/out/Products/<Configuration>-android), so when several Android ABIs are
        // built in one invocation give each its own scratch directory rather than let them
        // overwrite one another. Only do this when it is actually needed: a separate scratch
        // directory means a separate dependency resolution, which is wasted work for a
        // single-architecture build (and pathologically slow for package graphs that resolve
        // badly, such as one carrying two URLs for the same package identity).
        let scratchPath = (buildSystem == .swiftbuild && separateScratchPerArch)
            ? toolchainOptions.scratchPath.map({ $0 + "/" + (tc.sdkName ?? tc.swiftSDKVersion) })
            : toolchainOptions.scratchPath

        if let destinationURL = tc.destinationURL {
            cmd += ["--destination", destinationURL.path]
        } else if let sdkName = tc.sdkName {
            cmd += swiftSDKArguments(bundlePath: tc.sdkBundlePath, triple: sdkName, scratchPath: scratchPath)
        }
        ...
        */
        ...
        if let destinationURL = tc.destinationURL {
            ...
            binPathCmd += swiftSDKArguments(bundlePath: tc.sdkBundlePath, triple: sdkName, scratchPath: scratchPath)
        }
        if let packagePath = toolchainOptions.packagePath {
        ...
        // query the same engine that performed the build, since each lays out products differently
        binPathCmd += ["--build-system", buildSystem.rawValue]
        ...
        return (cmd: cmd, env: env, binPath: binPath, buildOutputFallback: buildOutputFallback(buildSystem: buildSystem, scratchPath: scratchPath, tc: tc))
    }

    /// A `--build-system` value carried in the trailing arguments, if any.
    ///
    /// Gradle projects generated by older versions of Skip pin `--build-system native` in the
    /// `skip android build` command they run. That is a baked-in default rather than a choice the
    /// person running the build made, so it is only honored when no engine was selected explicitly.
    var passedThroughBuildSystemArgument: String? {
        args.firstIndex(of: "--build-system").flatMap({ $0 + 1 < args.count ? args[$0 + 1] : nil })
    }

    /// The trailing arguments with any `--build-system <value>` pair removed, so that a pinned value
    /// from a generated gradle file cannot conflict with the engine actually selected.
    var argsWithoutBuildSystem: [String] {
        guard let index = args.firstIndex(of: "--build-system") else { return args }
        var remaining = args
        remaining.removeSubrange(index...min(index + 1, args.count - 1))
        return remaining
    }

    /// The build engine to use for `swift build` invocations made with the given `swift` executable.
    ///
    /// Precedence: `SKIP_BUILD_SYSTEM` (set by outer commands such as `skip checkup`) overrides
    /// everything, because the generated gradle that launches this command pins its own
    /// `--build-system` value; then `--build-system` as parsed from the command line; then a value
    /// carried in the trailing pass-through arguments; then `auto`.
    func resolvedBuildSystem(swift swiftCommand: String) async -> SwiftBuildSystem {
        let forced = SwiftBuildSystem.fromEnvironment()
        if forced != .auto {
            return forced
        }
        if toolchainOptions.buildSystem != .auto {
            return toolchainOptions.buildSystem
        }
        if let passedThrough = passedThroughBuildSystemArgument.flatMap(SwiftBuildSystem.init(rawValue:)) {
            return passedThrough
        }
        return await toolchainOptions.buildSystem.resolved(swift: swiftCommand)
    }

    /// The `swift build` arguments that select the Swift SDK Skip resolved.
    ///
    /// Several installed Android SDKs can offer the same target triple (e.g. a 6.3 and a 6.4 SDK
    /// both vending `aarch64-unknown-linux-android28`). Selecting by triple alone is ambiguous:
    /// SwiftPM 6.4 fails with "matched multiple SDKs", and earlier versions warn and pick one —
    /// not necessarily the bundle Skip resolved from `--swift-version`. Naming the bundle ID with
    /// `--swift-sdk <id> --triple <triple>` is not a fix either: SwiftPM then applies the bundle's
    /// *first* target triple's resource paths, so an aarch64 build links armv7 runtime libraries.
    ///
    /// Instead, scope SwiftPM's SDK search to a directory holding only the resolved bundle, and
    /// keep selecting by triple. The scope directory is a stable symlink under the scratch path,
    /// so it does not disturb incremental builds.
    func swiftSDKArguments(bundlePath: URL?, triple: String, scratchPath: String?) -> [String] {
        guard let bundlePath = bundlePath, let scopeDir = swiftSDKScopeDirectory(for: bundlePath, scratchPath: scratchPath) else {
            return ["--swift-sdk", triple]
        }
        return ["--swift-sdks-path", scopeDir.path, "--swift-sdk", triple]
    }

    /// Create (or refresh) a directory containing a single symlink to the given Swift SDK bundle.
    /// Returns `nil` if it cannot be created, in which case the caller falls back to an unscoped
    /// SDK search.
    func swiftSDKScopeDirectory(for bundlePath: URL, scratchPath: String?) -> URL? {
        let base = URL(fileURLWithPath: scratchPath ?? ((toolchainOptions.packagePath ?? ".") + "/.build"), isDirectory: true)
        let scopeDir = base.appendingPathComponent("swift-sdks", isDirectory: true)
        let link = scopeDir.appendingPathComponent(bundlePath.lastPathComponent, isDirectory: true)
        do {
            // clear out any previously-scoped bundle so a changed --swift-version takes effect,
            // leaving SwiftPM's own bookkeeping (e.g. the "configuration" folder it creates here) alone
            if isDir(scopeDir) {
                for existing in try FileManager.default.contentsOfDirectory(at: scopeDir, includingPropertiesForKeys: nil) {
                    if existing.pathExtension == "artifactbundle" && existing.lastPathComponent != bundlePath.lastPathComponent {
            ...
            if destination != bundlePath.path {
                try? FileManager.default.removeItem(at: link)
                try FileManager.default.createSymbolicLink(atPath: link.path, withDestinationPath: bundlePath.path)
            }
            return scopeDir
        } catch {
            return nil
    ...
    /// The build product directory to fall back on when `swift build --show-bin-path` cannot be queried.
    /// Each build engine has its own layout:
    ///
    ///     native:     <scratch>/<target-triple>/<configuration>
    ///     swiftbuild: <scratch>/out/Products/<Configuration>-android
    func buildOutputFallback(buildSystem: SwiftBuildSystem, scratchPath: String?, tc: ToolchainPaths) -> String {
        let buildConfig = toolchainOptions.configuration ?? BuildConfiguration.fromEnvironment() ?? .debug
        let scratch = scratchPath ?? ((toolchainOptions.packagePath ?? ".") + "/.build")
        switch buildSystem {
        case .swiftbuild:
            return [scratch, "out", "Products", buildConfig.rawValue.capitalized + "-android"].joined(separator: "/")
        case .native, .auto:
            let triple = tc.sdkName ?? ""
            return [scratch, triple, buildConfig.rawValue].joined(separator: "/")
        }
    }
    ...
    func runSwiftPM(cleanup: Bool? = nil, execute executable: String? = nil, commandEnvironment: [String] = [], defaultArch: AndroidArchArgument, remoteFolder: String? = nil, copy: [String] = [], archiveOutputFolder: URL? = nil, testingLibrary: TestingLibrary?, with out: MessageQueue) async throws {
        ...
        for arch in architectures {
            ...
            var (_, env, binPath, buildOutputFallback) = try await runToolchainCommand(tc, executable: executable, testMode: runTests ? .executable : nil, separateScratchPerArch: architectures.count > 1, with: out)
    ...
    func buildToolchainConfiguration(for arch: AndroidArch) throws -> ToolchainPaths {
        ...
        return ToolchainPaths(toolchainPath: toolchainPath, swiftSDKVersion: swiftSDKVersion, destinationURL: nil, sdkName: sdkName, sdkBundlePath: sdkPath, libPathDynamic: libPathDynamic, libPathStatic: libPathStatic, libSysrootArch: libSysrootArch, sysrootDir: sysrootDir)
    }
...
struct ToolchainOptions: ParsableArguments {
    ...
    // The SwiftPM default flipped from `native` to `swiftbuild` in Swift 6.4, and the two
    // engines lay out their build products differently, so Skip pins this rather than
    // inheriting the toolchain default. See SwiftBuildSystem.
    // SKIP_BUILD_SYSTEM overrides this (see SwiftBuildSystem.environmentKey); it is not merely the
    // default here, because generated gradle pins an explicit value that would otherwise win.
    @Option(help: ArgumentHelp("SwiftPM build system to use", valueName: "auto|native|swiftbuild"))
    var buildSystem: SwiftBuildSystem = .auto
...
struct ToolchainPaths {
    ...
    /// The target triple of the Swift SDK, e.g. `aarch64-unknown-linux-android28`
    let sdkName: String?
    /// The `.artifactbundle` directory of the Swift SDK that Skip resolved. More than one
    /// installed Android SDK can offer the same target triple (e.g. a 6.3 and a 6.4 SDK both
    /// vending `aarch64-unknown-linux-android28`); SwiftPM 6.4 rejects that outright and
    /// earlier versions silently pick one, which may not be the one Skip selected. Scoping
    /// SwiftPM's SDK search to this bundle removes the ambiguity.
    let sdkBundlePath: URL?
